...
### TASK 1 
mc_menu_data$Category <- as.factor(mc_menu_data$Category)
#1.1 Отобрать все данные по категории "Snacks & Sides" (2 балла)
mc_menu_data[mc_menu_data$Category == 'Snacks & Sides', ]
#1.2 Отобрать все данные по категориям "Desserts" и "Salads" (2 балла)
mc_menu_data[mc_menu_data$Category == 'Desserts' | mc_menu_data$Category == 'Salads', ]
#1.3 Отобрат все данные кроме категории "Breakfast" (2 балла)
mc_menu_data[mc_menu_data$Category != 'Breakfast', ]
#1.4 Посчитать число блюд, где калорийность больше чем 500 (2 балла)
nrow(mc_menu_data[mc_menu_data$Calories > 500, ])
#1.5 Найти самое калорийное блюдо (2 балла)
mc_menu_data[which.max(mc_menu_data$Calories),c(2,4)]
...
md %>% 
  filter(Vitamin.A....Daily.Value.== 0, Vitamin.C....Daily.Value. == 0) %>% 
  group_by(Category) %>% 
  summarise(length(Category))
...
md %>% 
  ...
  mutate(Fat_part = format(Calories.from.Fat/Calories, digits = 3))
#2.3 Посчитайте среднее значение, медиану и разницу между ними для холестерола (2 балла)
md %>% summarise(mean_Chol = mean(Cholesterol), median_Chol = median(Cholesterol), 
                 mean_Chol_minus_median_Chol = mean(Cholesterol)-median(Cholesterol))
#2.4 Для каждой категории найдите блюдо с самым высоким отношением сахаров к углеводам (2 балла)
md %>% mutate(Sug_Carb = Sugars/Carbohydrates) %>% 
  group_by(Category) %>% 
  summarise(Item[which.max(Sug_Carb)])
...
ggplot(leafshape, aes(x = location, y = bladelen, color = arch)) + 
  geom_boxplot() + 
  labs(x = "Location", y = "Leaf length", title = "Leaf length distribution, depending on location and architecture", 
       color = "Architecture") + 
...
ggplot(leafshape, aes(x = location, y = bladelen, fill = location)) + 
  geom_violin() + 
  facet_grid(cols = vars(arch)) + 
  labs(x = "Location/Architecture", y = "Leaf length", 
       title = "Leaf length distribution, depending on location and architecture", 
       fill = "Location") + 
  theme_bw() + 
